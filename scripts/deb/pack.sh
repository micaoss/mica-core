#!/usr/bin/env bash
# Turn a staged tree into one Debian package. Runs inside the target
# architecture's mica-build-env base image (dpkg-shlibdeps resolves the libraries
# of the container it runs in), called from a producer Dockerfile.
#
#   pack.sh --root <dir> --control <template> --version <v> --arch <amd64|arm64>
#           --out <dir> [--maintainer-scripts <dir>]
#
# Requires SOURCE_DATE_EPOCH, MICA_DEB_SOURCE_REPO and MICA_DEB_SOURCE_COMMIT.
# The template uses @VERSION@ and @ARCH@ and may use ${shlibs:Depends}; this
# script writes Installed-Size, md5sums, Mica-Source-Repo and
# Mica-Source-Commit, sets every mtime to SOURCE_DATE_EPOCH, owns everything by
# root, and verifies the archive against the staged tree.
set -euo pipefail
...
while [ "$#" -gt 0 ]; do
    ...
    --arch) ARCH="${2-}"; [ -n "${ARCH}" ] || die "--arch takes amd64 or arm64"; shift 2 ;;
    ...
    *) die "'$1' is not an option this packer takes" ;;
    esac
done
...
case "${ARCH}" in
amd64 | arm64) ;;
*) die "--arch ${ARCH} is not amd64 or arm64" ;;
esac

CONTAINER_ARCH="$(dpkg --print-architecture)"
if [ "${ARCH}" != "${CONTAINER_ARCH}" ]; then
    die "--arch ${ARCH} in a ${CONTAINER_ARCH} container. dpkg-shlibdeps resolves the libraries of the architecture it RUNS on, so this would record ${CONTAINER_ARCH}'s dependency versions in an ${ARCH} package and fail on the device, not here. Run this inside the ${ARCH} manifest of the mica-build-env base image"
fi
...
for f in Package Version Architecture Maintainer Section Priority Description; do
    [ -n "$(control_field "${f}" "${CONTROL}")" ] ||
        die "--control ${CONTROL} declares no ${f}. Package, Version, Architecture, Maintainer, Section, Priority and Description are required"
done
