...
IMAGE=$(bash "$REPO/scripts/build/from.sh" --arch=amd64 --ref rust)
HOST_REPO=$REPO
...
cargo test --locked --offline -p mica-deploy --lib boot::shutdown::tests
cargo test --locked --offline -p mica-deploy --test shutdown --test exitrd --test startup --test boot
...
for compiler in "${compilers[@]}"; do
    "$compiler" -std=c11 -Wall -Werror -c /src/scripts/gate/boot-shutdown/uapi.c -o "/target/b3-uapi-$compiler.o"
