//! The structural redactor every value leaving the two read-only roots
//! passes through.
//!
//! The list is fail-open: a secret-bearing field under a name it does not
//! carry is served. The mitigation is a test rather than a hope, and
...
/// The field names whose values never leave the device.
///
...
///
/// **The rule that makes that luck into a rule**.
...
///
/// The structural mirror of [`walk`], and it exists for one round trip: *"A redacted field is **read-only through the API**: a `PUT` whose
...
///  A client that reads a subtree, edits one
