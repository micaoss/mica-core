//! apid's half of the device audit trail: one
...
impl Audit {
    ...
    /// actor is [`ACTOR_OPERATOR`] — the distinction that matters is
    ...
    ///
    /// A failed write is logged and swallowed, deliberately: the ideal is
    /// "no audit trail ⇒ no shell", but refusing to serve management when the
    /// disk fails is a lockdown decision not taken here. The
    /// journal mirror is emitted first so a failing disk cannot silence the event entirely.
...
mod tests {
    ...
    fn lines_are_jsonl_with_the_documented_fields_and_mode_0600() {
        ...
        // The fifth member. A route reached this sink, so a human asked.
