...
// Status read: complete and tested, no route exposes it yet.
#[allow(dead_code)]
#[derive(Debug, Clone, PartialEq, Eq)]
...
impl Recheck {
    ...
    /// intersection — the sole deactivation trigger. A bundle with no
    ...
    // which IS called, so the two triggers stay readable as a pair.
    #[allow(dead_code)]
    #[must_use]
...
impl Store {
    ...
    /// Phase 4's second sanctioned local install path: apid picks up a
...
mod tests {
    ...
    /// Phase 4's second local install path: apid picks up a staged
    /// directory.
    #[test]
