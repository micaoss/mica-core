//! Request-path resolution for a static asset bundle.
//!
...
//! built-in assets. Together they enforce the rules:
...
pub enum Rejection {
    ...
    /// rule is "no symlinks in bundle contents", not "no
    /// escaping symlinks".
    Symlink,
...
impl Rejection {
    /// Whether the asset router may answer this with the SPA fallback.
    ///
    /// Only [`Rejection::NotFound`] may: a request the guards rejected must
    /// never come back as `200 text/html`. The fallback conditions alone do not
...
mod tests {
    ...
    ///
    /// The rows are deliberately *not* the compound attack strings.
    ...
    ///
    /// This is the assertion a compound string cannot make. If the
    ...
    fn ordinary_paths_resolve() {
        ...
        for (input, expected) in [
            ...
            // content-hashed name is the shape the immutable class exists
            // for.
    ...
    /// A miss is not a hostile request, and it is the one outcome the SPA
    /// fallback may answer.
    #[test]
    ...
    /// widening the assertion, which is what the "resolved bundle root"
