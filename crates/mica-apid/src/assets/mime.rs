...
//! - [`cache_class`] places a file in one of the classes.
...
///
/// `immutable_dir` is the directory the optional `mica-ui.json` declares
...
/// is the one that would silently break the operator criterion — an
...
pub fn cache_class(relative_path: &Path, immutable_dir: Option<&Path>) -> CacheClass {
    ...
    match immutable_dir {
        ...
        // immutable by accident; immutability has to be opt-in for a
        // named directory.
...
mod tests {
    ...
    /// These two are named specifically: they break rather than degrade on
    ...
    /// The operator criterion, at this layer. An `index.html` that landed
