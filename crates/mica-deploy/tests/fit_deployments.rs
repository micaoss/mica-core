...
fn fixture() -> (TempDir, DeploymentStore, BootReceipt, PathBuf) {
    ...
    for (slot, offset) in boards::layout("cx3576").offsets().into_iter().enumerate() {
...
fn fit_content_failure_retires_only_an_uncounted_boot_with_a_fallback() {
    ...
    assert_eq!(
        Environment::load(&firmware, boards::layout("cx3576"))
            .unwrap()
            .records[0]
            .tries_left,
        Some(2)
    ...
    let env = Environment::load(&firmware, boards::layout("cx3576")).unwrap();
...
fn fit_confirmation_and_rollback_preserve_the_loader_and_fallback_counter() {
    ...
    assert_eq!(
        before[..boards::layout("cx3576").offsets()[0] as usize],
        fs::read(&firmware).unwrap()[..boards::layout("cx3576").offsets()[0] as usize]
    );
}
...
fn fit_confirmation_requires_the_running_kernel_and_consumed_attempt() {
    ...
    let mut env = Environment::load(&firmware, boards::layout("cx3576")).unwrap();
