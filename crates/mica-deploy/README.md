...
the `mica-build-env` images named in `locks/mica-build-env.lock` (the release asset, committed unchanged):
...
make publish         # release workflow only: TAG=<YYYYMMDD-HHMM>, the archives as that GitHub Release's assets
```
...
assembly (`micaoss/mica-build`) imports both through `deps/packages/` and
builds neither.
...
mica-deploy import /path/to/update.micaupd
