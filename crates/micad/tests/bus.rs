//! Integration test: exercise `com.mica.micad1` over a private session bus.
//!
//! Spawns a private `dbus-daemon --session` plus the `micad` binary in
...
/// `root` locked the way `mica-shadow-reconcile` leaves it.
...
async fn bus_roundtrip() -> anyhow::Result<()> {
    ...
    // render a configuration nobody chose.
    let config_dir = dir.path().join("config");
    std::fs::create_dir_all(&config_dir)?;
    let mut seeded = micad_settings::Settings::default();
    seeded.provisioning.device_id = Some(DEVICE_ID.to_string());
    micad_settings::Store::new(&settings_path, &config_dir).save(&seeded)?;
    ...
    // No tree-wide `schema_version`: the version is on each
    // document, so the addressed tree carries none at all.
    ...
    // `hostname` is carried by `/mica/config/system.json` ,
    ...
    assert!(
        ...
        "device identity is runtime configuration for mqttd, not a micad D-Bus member:\n{xml}"
    );
    ...
    assert_eq!(error_name(&err), "com.mica.micad1.Error.NotFound");
    let err = proxy
    ...
    // The diagnostic reads, by member name for the same reason, and each
