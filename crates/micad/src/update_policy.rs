...
//! [`micad_settings::configuration`], because apid reports the effective
...
use micad_settings::configuration::{self, BakedUpdate};
...
pub use micad_settings::configuration::{
...
impl LoadedPolicy {
    ...
    pub fn auto_check_minutes(&self) -> Option<u64> {
        ...
        match selection.mode {
            ...
            // install steps behind it are the slice.
...
impl PolicyStore {
    ...
    /// Attach the baked layer read from the image.
    #[must_use]
...
pub struct GateVerdict {
    ...
    /// The health-path vocabulary: one code per entry of `reasons`,
...
mod tests {
    ...
    fn a_blocking_health_report_closes_the_gate_and_an_override_lifts_it() {
        ...
        // The health path: the class is a fixed word and the
