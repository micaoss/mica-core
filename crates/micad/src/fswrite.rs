...
/// file in micad needs an owner (the shadow file transient.rs rewrites) and the
...
fn write_via_rename(
    ...
    let temp = directory.join(format!(".{file_name}.micad-tmp"));
...
mod tests {
    ...
    fn a_path_that_does_not_exist_is_not_taken_for_a_mount_point() {
        ...
        // stop being atomic for every file micad renders.
        let dir = tempfile::tempdir().expect("tempdir");
