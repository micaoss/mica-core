...
/// Account whose password hash micad owns.
...
/// Shadow path micad operates on: [`SHADOW_ENV`] if set, else
/// [`DEFAULT_SHADOW`].
...
/// `mica-shadow-reconcile` tests on the next boot, so the two agree about what
/// counts as set.
...
/// until the next boot, when `mica-shadow-reconcile` clears both.
pub fn set_transient_root_password(shadow_path: &Path, password: &str) -> Result<()> {
    ...
    // (that branch is what lets a hash micad did not write survive): a password
...
mod tests {
    ...
    fn the_marker_sits_beside_the_shadow_file_whatever_that_is() {
        assert_eq!(
            transient_marker_path(Path::new("/var/lib/mica/shadow")),
            Path::new("/var/lib/mica/transient-root-password")
        );
    ...
    fn active_is_false_before_true_after_and_false_once_the_marker_goes() {
        ...
        // What mica-shadow-reconcile does on the next boot.
